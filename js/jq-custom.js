	...
	var x = 0;//eto ung index na laging tatanggaling sa questionsArray[] , 0 kase laging unang index lang tatanggalen nten;
			...
			var localQuizArrayName = "spaceTest";// iba dapat kada game para sa function na storeQuizArrayToLocalStorage
	...
	*/
	...
	function start(){
		...
		setTimeout(function(){
		myTimer = setInterval(function(){myTimer1()},3000);
		});
	}
	...
	function myRsgTimer1(){
		...
		rsg--;
		if(rsg == 3){
			$("#rsgTimer").html("YOUR TIMER");
		
		}
		if(rsg == 2){
			$("#rsgTimer").html("STARTS");
		}
		if(rsg == 1){
			$("#rsgTimer").html("NOW!!!");
		}
	...
	function checkIfLogin(){
		if(localStorageUsername == "" || localStorageUsername == null){
			...
			$("#txt-username").text("Welcome to Space World \n"+localStorageUsername);
	...
	function postScoreOnline(){		
		...
		if(a == null || a == ""){
			$("#game_over_page_msg").html("You need to <a class='btn-login' href='#stack2' data-toggle='modal'>login</a> first!");
			//alert("You should login to post online");
			//location.href = "login_page.html";
		}else if(a == "Snap!Internet Connection Failed!"){
			...
			$.ajax({
				...
				success: function(data){
					if(data=="not"){
						...
						$("#game_over_page_msg").html(" Game Failed");
					}else if(data=="success"){
					
						$("#game_over_page_msg").html("Your Score was posted online. Thanks for playing! :)");
